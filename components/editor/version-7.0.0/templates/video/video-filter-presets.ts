 ...
 *
 * This file defines preset CSS filters that can be applied to video overlays.
 * Each preset represents a specific visual style that can be selected from the UI.
 * The filter strings are valid CSS filter values to be directly applied to the video element.
 */
 ...
 */
export const getPresetById = (presetId: string): VideoFilterPreset => {
  return (
    VIDEO_FILTER_PRESETS.find((preset) => preset.id === presetId) ||
    VIDEO_FILTER_PRESETS[0]
